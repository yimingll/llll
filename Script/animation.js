...
class Animation {
  ...
  activate(drawX, drawY, lightX, lightY) {
    ...
    this.updateFrameParameters(this.contexts, this.index)
    this.anchorX = lightX
    this.anchorY = lightY
  }
    ...
    if (emitter === undefined) {
      ...
      emitter.update = deltaTime => {
        ...
        if (this.layer.position === 'relative') {
          const tw = Scene.binding.tileWidth
          const th = Scene.binding.tileHeight
          const ox = (position.x - emitter.x) * tw
          const oy = (position.y - emitter.y) * th
