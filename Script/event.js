...
const EventManager = new class {
  ...
  initialize() {
    ...
    // 编译事件指令
    for (const {id, enabled, type, commands} of events) {
    ...
    if (data.length > 0) {
      for (const wrap of data) {
        ...
        if (script === undefined) {
          ...
          console.warn(`The script file is missing: ${name}`, owner)
          continue
        }
        // 创建脚本对象实例，并传递脚本参数
