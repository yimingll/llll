...
class Printer {
  ...
  matchTag() {
    ...
    if (match = string.match(regexps.colorIndex)) {
      ...
      const color = Color.parseInt(hex)
      this.drawBuffer()
  ...
  static loadDefault() {
    ...
    this.size = 16
    this.color = Color.parseInt('ffffffff')
    this.effect = {type: 'none'}

    // 设置像素化阈值
    if (font.pixelated) {
      const program = GL.textProgram.use()
      GL.uniform1f(program.u_Threshold, font.threshold / 255)
    }

    // 导入字体
    return this.importFonts(font.imports)
  }
  ...
  static importFonts(imports) {
    ...
    for (const guid of imports) {
      const path = File.getPathByGUID(guid)
  ...
  static parseEffect(effect) {
    ...
    if (copy.color !== undefined) {
      copy.color = Color.parseInt(copy.color)
    }
    return copy
  }
