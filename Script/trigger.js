...
class Trigger {
  ...
  static getActorsBySelector(caster, selector) {
    ...
    if (caster) {
      ...
      const relations = Team.relationMaps[caster.teamId]
      const length = actors.count
      for (let i = 0; i < length; i++) {
        const actor = actors[i]
        if (inspector(caster, actor, relations)) {
          actors[count++] = actor
        }
      }
    }
  ...
  static actorGetters = new class {
    ...
    'circle' = (x, y, angle, scale, shape) => {
      ...
      for (let i = 0; i < length; i++) {
        ...
        for (let i = 0; i < length; i++) {
          const actor = actors[i]
          // 如果角色已激活，且角色的锚点位于圆形触发区域中，则添加到目标列表中
          if (actor.active && (x - actor.x) ** 2 + (y - actor.y) ** 2 < radius ** 2) {
            targets[count++] = actor
          }
        }
      }
        ...
        for (let i = 0; i < length; i++) {
          ...
          if (actor.active) {
            ...
            if (rx ** 2 + ry ** 2 < radius ** 2) {
              ...
              const half = shape.centralAngle / 2
