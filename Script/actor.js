...
const Party = new class {
    ...
    if (Data.config.actor.partyInventory === 'shared') {
      if (this.player instanceof GlobalActor) {
        for (const actor of this.members) {
          actor.inventory = this.player.inventory
        }
      }
    }
  }
  ...
  loadData(party) {
    ...
    for (const member of members) {
      ...
      if (actor) this.members.push(actor)
    }
  }
}
...
const Team = new class {
  ...
  /** 初始化 */
  initialize() {
    this.keys = Data.teams.list.map(team => team.id)
    this.relationMaps = this.buildRelationMaps(this.keys, Data.teams.relations)
    this.defaultId = this.keys[0]
  }
  ...
  changeRelation(teamId1, teamId2, relation) {
    if (teamId1 !== teamId2 &&
      this.relationMaps[teamId1] &&
      this.relationMaps[teamId2]) {
      this.relationMaps[teamId1][teamId2] = relation
      this.relationMaps[teamId2][teamId1] = relation
    }
  }

  /** 保存队伍关系数据 */
  saveData() {
    const map = this.relationMaps
  ...
  loadData(team) {
    ...
    const sMap = this.buildRelationMaps(sKeys, team.relations)
    const dKeys = this.keys
    const dMap = this.relationMaps
    // 将加载的队伍关系数据合并到现有的数据中
    // 丢弃项目编辑所造成的无效数据
    for (const key of dKeys) {
      const sRelations = sMap[key]
      if (!sRelations) continue
      const dRelations = dMap[key]
      for (const key of dKeys) {
...
const ActorManager = new class {
  ...
  create(actorId, savedData) {
    ...
    if (!this.idMap[actorId] && data) {
      ...
      actor.entityId = savedData?.entityId ?? this.generateEntityId()
      // 发送自动执行事件
      if (savedData === undefined) {
        actor.emit('autorun')
      }
      this.entityIdMap[actor.entityId] = actor
...
class Actor {
  ...
  constructor(data, savedData) {
    ...
    if (savedData) {
      ...
      this.collider.setScale(savedData.scale)
   ...
   */
  setTeam(teamId) {
    if (teamId in Team.relationMaps) {
      this.teamId = teamId
    }
  }
  ...
  static inspectors = new class {
    // 检查器 - 判断敌对角色
    'enemy' = (a, b, relations) => {
      return relations[b.teamId] === 0
    }

    // 检查器 - 判断友好角色
    'friend' = (a, b, relations) => {
      return relations[b.teamId] === 1
    }
    ...
    // 检查器 - 判断小队角色除自己以外
    'team-except-self' = (a, b, relations) => {
      return a !== b && a.teamId === b.teamId
    }

    // 检查器 - 判断任意角色除自己以外
    'any-except-self' = (a, b, relations) => {
      return a !== b
    }
...
class StateCountdownList extends Array {
  ...
  update(deltaTime) {
    ...
    while (--i >= 0) {
      // 如果倒计时结束，则将状态从列表中移除
      if ((this[i].currentTime -= deltaTime) <= 0) {
        this[i].currentTime = 0
        this.manager.remove(this[i])
      }
    }
  }
}
...
class Inventory {
  ...
  increaseItems(id, quantity) {
    ...
    if (item) item.increase(quantity)
    else this.createItems(id, quantity)
  }
...
class TargetManager {
  ...
  detect(distance, inspector, inSight = false) {
    ...
    const ox = owner.x
    const oy = owner.y
    const relations = Team.relationMaps[owner.teamId]
    // 获取探测范围所在的角色区间列表
    ...
    for (let i = 0; i < count; i++) {
      ...
      for (let i = 0; i < length; i++) {
        ...
        if (actor.active && (ox - actor.x) ** 2 + (oy - actor.y) ** 2 <= square &&
          inspector(owner, actor, relations) && (inSight === false ||
  ...
  discard(inspector, distance = 0) {
    ...
    while (--i >= 0) {
      ...
      if (inspector(owner, actor, relations) && (ox - actor.x) ** 2 + (oy - actor.y) ** 2 >= square) {
        this.remove(actor)
      }
    }
  }
  ...
  getTargetMaxThreat(inspector) {
    ...
    for (let i = 0; i < length; i++) {
      const actor = targets[i]
      // 检查角色关系，并找出最大仇恨值的目标
      if (inspector(owner, actor, relations)) {
  ...
  getTargetRandom(inspector) {
    ...
    for (let i = 0; i < length; i++) {
      ...
      if (inspector(owner, targets[i], relations)) {
        indices[count++] = i
      }
    }
    if (count !== 0) {
...
class ActorCollider {
  ...
  // 设置缩放系数
  setScale(scale) {
    this.scaledSize = this.size * scale
    this.scaledHalf = this.scaledSize / 2
  }
  ...
  // 忽略队友碰撞开关
  static ignoreTeamMemberCollision = true
  ...
  static initialize() {
    ...
    // 设置忽略队友碰撞开关
    this.ignoreTeamMemberCollision = collision.actor.ignoreTeamMember
    // 设置场景碰撞系统开关
  ...
  static handleCollisionBetweenTwoActors = (IIFE => {
    ...
    return (sActor, dActor) => {
      ...
      // 如果开启了忽略队友碰撞
      if (this.ignoreTeamMemberCollision && sActor.teamId === dActor.teamId) return
      const sCollider = sActor.collider
      const distMin = sCollider.scaledHalf + dCollider.scaledHalf
...
class ActorNavigator {
   ...
   */
  navigateTo(x, y) {
    this.route(ScenePathFinder.createPath(this.actor.x, this.actor.y, x, y, this.actor.passage))
  }
   ...
   */
  followCircle(target, minDist, maxDist, offset = 0, navigate = false, once = false) {
    // 正在跟随相同目标时，返回
    if (this.target !== target) {
      ...
      ? this.#circleFollowTargetOnce
      : this.#circleFollowTarget
      // 设置更新函数为：跟随角色
   ...
   * @param {boolean} [once = false] 跟随一次(到达位置后停止移动)
   */
  followRectangle(target, minDist, maxDist, vertDist = 0, navigate = false, once = false) {
    // 正在跟随相同目标时，返回
    if (this.target !== target) {
      if (this.target !== target) {
        this.stopMoving()
      ...
      this.followOnce = once
      this.followTarget = once
      ? this.#rectangleFollowTargetOnce
      : this.#rectangleFollowTarget
      // 设置更新函数为：跟随角色
      this.update = this.followTarget
    }
  }
  ...
  updatePathMovement(deltaTime) {
    ...
    // 更新移动超时，避免卡墙角时停不下来
    this.updateMoveTimeout(deltaTime)
  }
   ...
   * @param {number} deltaTime 增量时间(毫秒)
   */
  updateMoveTimeout(deltaTime) {
    const actor = this.actor
    const x = Math.floor(actor.x)
    const y = Math.floor(actor.y)
    // 如果角色停留在一个网格内3秒，则停止移动
    if (x === this.lastX && y === this.lastY) {
      if ((this.timeout += deltaTime) > 3000) {
        this.timeout = 0
        this.stopMoving()
      }
    } else {
   ...
   */
  #switchToFollowTargetBuffer() {
    if (this.followOnce) {
      this.stopMoving()
    } else {
      this.update = this.#followTargetBuffer
  ...
  #circleFollowTarget(deltaTime) {
    ...
    if (dist > this.maxDist) {
      ...
      ? this.#circleNavigateToTarget
      : this.#circleApproachTarget
      return this.update(deltaTime)
    }
    // 如果角色距离小于最小距离，开始跟随
    // 设置更新函数为：远离目标(圆形模式)
    if (dist < this.minDist) {
      actor.animationController.startMoving()
      this.update = this.#circleLeaveTarget
      return this.update(deltaTime)
    }
  }
   ...
   * @param {number} deltaTime 增量时间(毫秒)
   */
  #circleFollowTargetOnce(deltaTime) {
    this.#circleFollowTarget(deltaTime)
    // 如果当前跟随方法就是更新函数，表示跟随已经结束
    if (this.#circleFollowTargetOnce === this.update) {
      this.stopMoving()
    }
  }
   ...
   * @param {number} deltaTime 增量时间(毫秒)
   */
  #circleApproachTarget(deltaTime) {
    const actor = this.actor
    const target = this.target
    // 如果目标已销毁，停止跟随
    if (target.destroyed) {
    ...
    if (Math.sqrt(distX ** 2 + distY ** 2) <= this.maxDist) {
      return this.#switchToFollowTargetBuffer()
    }
   ...
   * @param {number} deltaTime 增量时间(毫秒)
   */
  #circleNavigateToTarget(deltaTime) {
    const actor = this.actor
    const target = this.target
    // 如果目标已销毁，停止跟随
    if (target.destroyed) {
    ...
    if (Math.sqrt(distX ** 2 + distY ** 2) <= this.maxDist) {
      this.movementPath = null
      this.#switchToFollowTargetBuffer()
      return
    }
    // 每隔一段时间计算移动路径
    if (!this.movementPath || (this.timeout -= deltaTime) <= 0) {
      ...
      this.movementPath = ScenePathFinder.createPath(sx, sy, x, y, actor.passage)
      this.timeout = 500
    }
    // 逐帧计算角度，并计算移动速度分量
    const path = this.movementPath
   ...
   * @param {number} deltaTime 增量时间(毫秒)
   */
  #rectangleFollowTarget(deltaTime) {
    const actor = this.actor
    const target = this.target
    // 如果目标已销毁，停止跟随
    if (target.destroyed) {
    ...
    if (distX > this.maxDist ||
      ...
      actor.animationController.startMoving()
      this.update = this.followingNavigate
      ? this.#rectangleNavigateToTarget
      : this.#rectangleApproachTarget
      return this.update(deltaTime)
    }
  }
   ...
   * @param {number} deltaTime 增量时间(毫秒)
   */
  #rectangleFollowTargetOnce(deltaTime) {
    this.#rectangleFollowTarget(deltaTime)
    // 如果当前跟随方法就是更新函数，表示跟随已经结束
    if (this.#rectangleFollowTargetOnce === this.update) {
      this.stopMoving()
    }
  }
   ...
   * @param {number} deltaTime 增量时间(毫秒)
   */
  #rectangleApproachTarget(deltaTime) {
    const actor = this.actor
    const target = this.target
    // 如果目标已销毁，停止跟随
    if (target.destroyed) {
   ...
   * @param {number} deltaTime 增量时间(毫秒)
   */
  #rectangleNavigateToTarget(deltaTime) {
    const actor = this.actor
    const target = this.target
    // 如果目标已销毁，停止跟随
    if (target.destroyed) {
    ...
    // 每隔一段时间计算移动路径
    if (!this.movementPath || (this.timeout -= deltaTime) <= 0) {
      this.movementPath = ScenePathFinder.createPath(sx, sy, dx, dy, actor.passage)
      this.timeout = 500
    }
    // 逐帧计算角度，并计算移动速度分量
    const path = this.movementPath
