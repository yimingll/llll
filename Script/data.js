...
const Data = new class {
    ...
    if (manifest.deployed) {
      ...
      for (const key of [
        ...
        for (const [guid, data] of Object.entries(group)) {
          propDescriptor.value = guid
          defineProperty(data, 'id', propDescriptor)
        }
      }
    } else {
      // 加载未打包的数据
      ...
      for (const key of ['skills', 'items', 'equipments']) {
        ...
        for (const {guid, path} of manifest[key]) {
          ...
          if (item !== undefined) {
            ...
            propDescriptor.value = path.slice(index).replace(guidAndExt, '')
            defineProperty(item, 'filename', propDescriptor)
          }
        }
      }
    }
  ...
  loadScene(id) {
    ...
    if (this.scenes[id]) {
      return new Promise(resolve => {
        resolve(Codec.decodeScene(this.scenes[id]))
      })
    }
    ...
    return File.get({
      ...
      return Codec.decodeScene(this.scenes[id] = code)
    })
  }
  ...
  compileEvents(data) {
    ...
    for (const event of data.events) {
      const commands = Command.compile(event.commands)
      typeMap[Enum.getValue(event.type) || event.type] = commands
    }
  ...
  async loadSaveMeta() {
    ...
    return Promise.all(promises).then(data => {
      ...
      for (let i = 0; i < length; i++) {
        ...
        if (data[i]) {
          list.push({
            name: filenames[i].replace(extname, ''),
            data: data[i],
          })
        }
      }
  ...
  saveGlobalData() {
    ...
    switch (Stats.shell) {
      case 'electron': {
        ...
        // 异步写入全局数据文件
        return require('fs').promises.writeFile(path, json)
  ...
  async loadGlobalData() {
    ...
    switch (Stats.shell) {
      ...
      case 'web': {
        ...
        this.globalData = data ?? {variables: {}}
        break
      }
    }
  }
